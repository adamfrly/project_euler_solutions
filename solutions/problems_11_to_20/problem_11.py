# Find the largest product of four adjacent numbers (vertical, horizontal, or diagonal) in a 20 by 20 grid
...
def largest_horizontal_product(grid):
    maximum = 0
    for i, line in enumerate(grid):
        for j in range(len(line) - 3):
            test = np.product(line[j:j+4])
            if test > maximum:
                maximum = test
    return maximum


def largest_vertical_product(grid):
    transposed_grid = list(map(list, zip(*grid)))
    maximum = largest_horizontal_product(transposed_grid)
    return maximum
...
horiz = largest_horizontal_product(data)
vert = largest_vertical_product(data)
diag = diagonal_natural(data, 4)
rev_diag = diagonal_reverse(data, 4)

print(max(horiz, vert, diag, rev_diag))